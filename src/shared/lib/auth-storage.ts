...
const AUTH_REFRESH_TOKEN_KEY = 'app.auth.refresh-token'
...
function normalizeAuthUser(rawValue: unknown): AuthUser | null {
  ...
  const name = getNullableString(record.name) ?? getNullableString(record.email) ?? 'Unknown user'
  ...
  return {
    ...
    profilePhotoPath:
      getNullableString(record.profilePhotoPath) ?? getNullableString(record.profile_photo_path),
    profilePhotoUrl:
      getNullableString(record.profilePhotoUrl) ?? getNullableString(record.profile_photo_url),
  } as AuthUser
}
