...
import {
  ...
  isFixedAdminTablePath,
} from '@/app/router/route-presentation'
...
export function AppShellLayout() {
  ...
  const shouldLockPageScroll = isFixedAdminTablePath(location.pathname)

  const pageTitle = useMemo(() => {
    const brandName = t('layout.brand.name')
    if (!presentation) return brandName
    return `${brandName} - ${t(presentation.labelKey)}`
  }, [presentation, t])
