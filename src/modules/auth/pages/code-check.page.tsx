...
import { verifyRegisterQuizy } from '@/modules/auth/services/quizy-auth-flow.services'
...
export default function CodeCheckPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await verifyRegisterQuizy({ phoneNumber, countryCallingCode, otpCode })
  ...
  return (
    <AuthVisualLayout title="Confirm code" description="Enter the 4-digit code sent to your phone.">
      <div className="space-y-5">
        <Input
          ...
          dir="ltr"
        />
        <Button type="button" loading={isSubmitting} disabled={isSubmitting || otpCode.length !== 4} onClick={handleSubmit} icon={<ShieldCheck />} className="h-12 w-full rounded-2xl bg-[#6949ff] text-white">
          Confirm
        </Button>
      </div>
    </AuthVisualLayout>
