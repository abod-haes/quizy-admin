...
const normalizeCountryCallingCode = (value: unknown): string | null => {
  const code = toStringValue(value).replace(/\s/g, '').trim()
  if (!code) return null
...
export const academicContentConfigs: Record<ContentCrudConfig['key'], ContentCrudConfig> = {
  ...
  managementUsers: {
    ...
    toPayload: (values) => ({ firstName: values.firstName, lastName: values.lastName || null, password: values.password || null, phoneNumber: values.phoneNumber || null, countryCallingCode: normalizeCountryCallingCode(values.countryCallingCode), role: 3, description: values.description || null, url: null, primaryImageId: null, fileIds: [] }),
  },
  quizzes: {
