  ...
  return (
    <section className="w-full space-y-6">
      ...
      <Dialog open={formState.open} onOpenChange={(open) => setFormState((current) => ({ ...current, open }))}>
        <DialogContent className="max-w-2xl"><DialogHeader><DialogTitle>{t(formState.mode === 'edit' ? 'form.editTitle' : 'form.createTitle', { entity: t(config.titleKey) })}</DialogTitle><DialogDescription>{t('form.description')}</DialogDescription></DialogHeader>
          <div className="grid gap-4 py-2">{config.fields.map((field) => {
            ...
            return <div key={field.name} className="space-y-2"><Label htmlFor={fieldId}>{t(field.labelKey)}</Label>{field.type === 'textarea' || field.type === 'json' ? <Textarea id={fieldId} value={getFieldTextValue(formState.values[field.name])} placeholder={field.placeholderKey ? t(field.placeholderKey) : undefined} onChange={(event) => updateField(field.name, event.target.value)} /> : field.type === 'checkbox' ? <label className="flex h-11 items-center gap-3 rounded-2xl border border-border bg-background px-4 text-sm"><input type="checkbox" checked={formState.values[field.name] === true} onChange={(event) => updateField(field.name, event.target.checked)} />{t(field.labelKey)}</label> : field.type === 'select' ? <select id={fieldId} value={getFieldTextValue(formState.values[field.name])} className="quizy-select-field" onChange={(event) => updateField(field.name, event.target.value)}><option value="">{t('form.selectPlaceholder')}</option>{options.map((option) => <option key={option.id} value={option.id}>{option.name}</option>)}</select> : field.type === 'multi-select' ? <select id={fieldId} multiple value={getFieldArrayValue(formState.values[field.name])} className="quizy-select-field min-h-28" onChange={(event) => updateField(field.name, Array.from(event.currentTarget.selectedOptions).map((option) => option.value))}>{options.map((option) => <option key={option.id} value={option.id}>{option.name}</option>)}</select> : <Input id={fieldId} type={field.type === 'number' ? 'number' : 'text'} value={getFieldTextValue(formState.values[field.name])} placeholder={field.placeholderKey ? t(field.placeholderKey) : undefined} onChange={(event) => updateField(field.name, field.type === 'number' ? Number(event.target.value) : event.target.value)} />}{fieldError ? <p className="text-sm text-destructive">{t(fieldError)}</p> : null}</div>
          })}</div>
          <DialogFooter><Button type="button" variant="outline" onClick={() => setFormState((current) => ({ ...current, open: false }))}>{t('actions.cancel')}</Button><Button type="button" disabled={saveMutation.isPending} onClick={handleSubmit}>{saveMutation.isPending ? <Loader2 className="size-4 animate-spin" /> : null}{t('actions.save')}</Button></DialogFooter>
        </DialogContent>
      </Dialog>
    </section>
  )
