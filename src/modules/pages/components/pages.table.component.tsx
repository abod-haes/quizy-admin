...
const columns = [
  { key: 'slug', labelKey: 'table.columns.slug', isDate: false },
  { key: 'is_published', labelKey: 'table.columns.isPublished', isDate: false },
  { key: 'created_by', labelKey: 'table.columns.createdBy', isDate: false },
  { key: 'updated_by', labelKey: 'table.columns.updatedBy', isDate: false },
...
export const PagesTableColumns = columns
...
type PagesTableProps = {
  ...
  canEdit?: boolean
  canDelete?: boolean
  onView?: (row: PagesEntity) => void
  onEdit?: (row: PagesEntity) => void
  onDelete?: (row: PagesEntity) => void
}

function renderCellValue(row: PagesEntity, column: (typeof columns)[number], locale: string): string {
...
export function PagesTable({
  ...
  const tableColumns = useMemo<DataTableColumn<PagesEntity>[]>(() => {
    const mapped: DataTableColumn<PagesEntity>[] = visibleColumns.map((column) => ({
      id: column.key,
      header: t(column.labelKey),
      renderCell: (row) => renderCellValue(row, column, i18n.language),
    }))

    const actions: TableRowActionItem<PagesEntity>[] = []
