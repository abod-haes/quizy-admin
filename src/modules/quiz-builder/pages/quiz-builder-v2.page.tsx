...
function emptyQuestion(): QuizQuestionPayload {
  return {
    title: '',
    hint: '',
    lessonIds: [],
...
function QuizBuilderEditor({
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      navigate(savedId ? `/quiz-builder?quizId=${encodeURIComponent(savedId)}` : '/quizzes', {
        replace: true,
      })
    },
    onError: (error) => {
  ...
  const updateAnswer = (
    ...
    answerIndex: number,
    patch: Partial<QuizAnswerPayload>,
  ) => {
    setPayload((current) => ({
      ...current,
        ...
        <>
          <Card className="rounded-3xl">
            ...
            <CardContent className="grid gap-4 md:grid-cols-2">
              ...
              <div className="space-y-2 md:col-span-2">
                <Label>{t('fields.linkedContent')}</Label>
                <MultiSelect
