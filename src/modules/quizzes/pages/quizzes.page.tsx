...
export default function QuizzesPage() {
  ...
  const quizzesQuery = useQuery({
    queryKey: ['quizzes-page', page, search, teacherId],
    queryFn: () => {
      const params: LegacyPaginationQuery & { search?: string; teacherId?: string } = {
        Page: page,
        PerPage: PAGE_SIZE,
        ...(search.trim() ? { search: search.trim() } : {}),
        ...(teacherId !== ALL_VALUE ? { teacherId } : {}),
      }
  ...
  return (
    <section className="flex h-full min-h-0 w-full flex-col gap-3 overflow-hidden">
      <PageHeader
        ...
        controls={<><CustomSelect className="h-9 min-w-44" value={teacherId} variant="filter" options={teacherOptions} onValueChange={(value) => { setTeacherId(String(value)); setPage(1) }} />{hasFilters ? <Button variant="outline" icon={<X />} onClick={() => { setSearch(''); setTeacherId(ALL_VALUE); setPage(1) }}>{t('quizzes.clearFilters')}</Button> : null}</>}
        actions={<><Button variant="outline" icon={<RefreshCcw />} onClick={() => void quizzesQuery.refetch()} disabled={quizzesQuery.isFetching}>{t('quizzes.refresh')}</Button><Button icon={<Plus />} onClick={() => navigate('/quiz-builder')}>{t('quizzes.add')}</Button></>}
      />
