...
type SessionFormState = {
  open: boolean
  mode: 'create' | 'edit'
  item: CourseSession | null
  values: SessionFormValues
  errors: Partial<Record<keyof SessionFormValues, string>>
}
...
function getCourseLabel(course: CourseOption): string {
...
export default function CourseSessionsPage() {
  ...
  const coursesQuery = useQuery({
    queryKey: ['course-sessions', 'courses'],
    queryFn: () => api.get<PagedResponse<CourseOption>>(API_ENDPOINTS.courses.list, { params: { Page: 1, PerPage: 1000 } }),
    staleTime: 1000 * 60 * 5,
  })

  const courseOptions = useMemo(
    () => (coursesQuery.data?.items ?? []).map((course) => ({ value: course.id, label: getCourseLabel(course) })),
    [coursesQuery.data?.items]
  )

  const sessionsQuery = useQuery({
    queryKey: ['course-sessions', selectedCourseId, page, DEFAULT_PAGE_SIZE],
    queryFn: () => api.get<PagedResponse<CourseSession>>(API_ENDPOINTS.courses.sessions(selectedCourseId), { params: { Page: page, PerPage: DEFAULT_PAGE_SIZE } }),
  ...
  const saveMutation = useMutation({
    mutationFn: async (values: SessionFormValues) => {
      ...
      if (formState.mode === 'edit' && formState.item?.id) {
        return api.patch<unknown, typeof payload>(API_ENDPOINTS.courseSessions.update(formState.item.id), payload)
      }
    ...
    onSuccess: async () => {
      ...
      setFormState({ open: false, mode: 'create', item: null, values: emptySessionValues, errors: {} })
      await queryClient.invalidateQueries({ queryKey: ['course-sessions', selectedCourseId] })
    },
  ...
  const openCreateForm = () => {
    setFormState({ open: true, mode: 'create', item: null, values: emptySessionValues, errors: {} })
  }

  const openEditForm = (item: CourseSession) => {
    setFormState({
      open: true,
      mode: 'edit',
      item,
  ...
  const updateField = <TKey extends keyof SessionFormValues>(field: TKey, value: SessionFormValues[TKey]) => {
    setFormState((current) => ({ ...current, values: { ...current.values, [field]: value }, errors: { ...current.errors, [field]: '' } }))
  }

  const handleSubmit = () => {
    if (!selectedCourseId) {
      toast.error(t('sessions.selectCourseFirst'))
      return
    }
  ...
  const handleDelete = (item: CourseSession) => {
    const confirmed = window.confirm(t('messages.deleteConfirm', { name: item.title || t('messages.item') }))
    if (!confirmed) return
    ...
    <section className="flex min-h-0 w-full flex-col gap-6 overflow-hidden">
      <div className="rounded-[2rem] border border-primary/10 bg-card p-6 shadow-sm sm:p-8">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="max-w-3xl space-y-3">
            ...
            <h1 className="text-3xl font-bold tracking-tight text-foreground">{t('modules.courseSessions.title')}</h1>
            <p className="text-base leading-7 text-muted-foreground">{t('modules.courseSessions.description')}</p>
          </div>
          <div className="flex flex-wrap gap-2">
            <Button type="button" variant="outline" onClick={() => sessionsQuery.refetch()} disabled={!selectedCourseId || sessionsQuery.isFetching}>
              <RefreshCcw className="size-4" />{t('actions.refresh')}
            </Button>
            <Button type="button" onClick={openCreateForm} disabled={!selectedCourseId}>
      ...
      <Card className="flex min-h-0 flex-1 flex-col rounded-3xl shadow-sm">
        <CardHeader className="shrink-0 gap-4">
          <div className="grid gap-3 lg:grid-cols-[minmax(18rem,26rem)_1fr] lg:items-end">
            <div className="space-y-2">
              <Label>{t('fields.course')}</Label>
              <CustomSelect
                value={selectedCourseId || undefined}
                placeholder={t('sessions.coursePlaceholder')}
        ...
        <CardContent className="flex min-h-0 flex-1 flex-col overflow-hidden">
          {!selectedCourseId ? (
            <div className="flex min-h-[22rem] flex-col items-center justify-center rounded-3xl border border-dashed border-border bg-muted/30 p-10 text-center">
            ...
            <div className="rounded-3xl border border-dashed border-border bg-muted/30 p-10 text-center">
              <BookOpenCheck className="mx-auto mb-3 size-10 text-muted-foreground" />
                ...
                <Table className="min-w-[760px]">
                  <TableHeader className="sticky top-0 z-10 bg-card/95 backdrop-blur">
                    <TableRow>
                      ...
                      <TableHead>{t('fields.isFree')}</TableHead>
                  ...
                  <TableBody>
                    {items.map((item) => (
                      <TableRow key={item.id}>
                        <TableCell className="max-w-[18rem] truncate">{item.title || '-'}</TableCell>
                        <TableCell className="max-w-[22rem] truncate">{item.description || '-'}</TableCell>
                        <TableCell>{typeof item.order === 'number' ? item.order : '-'}</TableCell>
                        <TableCell>{item.isFree ? '✓' : '—'}</TableCell>
                        <TableCell>
          ...
          {selectedCourseId ? (
            <div className="mt-4 flex shrink-0 flex-col gap-3 border-t border-border/70 pt-4 lg:flex-row lg:items-center lg:justify-between">
              <p className="text-sm text-muted-foreground">{t('pagination.range', { start: startItem, end: endItem, total: totalCount })}</p>
              <div className="flex flex-wrap items-center gap-2">
                <Button type="button" variant="outline" disabled={page <= 1 || sessionsQuery.isFetching} onClick={() => goToPage(page - 1)}>{t('pagination.previous')}</Button>
                <Badge variant="outline" className="rounded-full px-3">{t('pagination.summary', { page, totalPages })}</Badge>
                <Button type="button" variant="outline" disabled={page >= totalPages || sessionsQuery.isFetching} onClick={() => goToPage(page + 1)}>{t('pagination.next')}</Button>
              </div>
            </div>
          ) : null}
        </CardContent>
      ...
      <Dialog open={formState.open} onOpenChange={(open) => setFormState((current) => ({ ...current, open }))}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>{t(formState.mode === 'edit' ? 'form.editTitle' : 'form.createTitle', { entity: t('modules.courseSessions.title') })}</DialogTitle>
            <DialogDescription>{t('form.description')}</DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-2">
            <div className="space-y-2">
              <Label htmlFor="session-title">{t('fields.title')}</Label>
              <Input id="session-title" value={formState.values.title} onChange={(event) => updateField('title', event.target.value)} />
              {formState.errors.title ? <p className="text-sm text-destructive">{t(formState.errors.title)}</p> : null}
            </div>
            <div className="space-y-2">
              <Label htmlFor="session-description">{t('fields.description')}</Label>
              <Textarea id="session-description" value={formState.values.description} onChange={(event) => updateField('description', event.target.value)} />
            </div>
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2">
                <Label htmlFor="session-order">{t('fields.order')}</Label>
                <Input id="session-order" type="number" value={String(formState.values.order)} onChange={(event) => updateField('order', Number(event.target.value))} />
                {formState.errors.order ? <p className="text-sm text-destructive">{t(formState.errors.order)}</p> : null}
              </div>
              <div className="space-y-2">
                ...
                <label className="flex h-11 items-center gap-3 rounded-2xl border border-border bg-background px-4 text-sm">
                  <input type="checkbox" checked={formState.values.isFree} onChange={(event) => updateField('isFree', event.target.checked)} />{t('fields.isFree')}
                </label>
              </div>
            </div>
          </div>
