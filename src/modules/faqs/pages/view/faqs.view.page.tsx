...
import { useFaqsById } from '@/modules/faqs/hooks/use-faqs-crud.hook'
...
export default function FaqsViewPage() {
  ...
  const { id } = useParams()
  const detailQuery = useFaqsById(id ?? '', Boolean(id))
    ...
    <section className="flex min-h-0 flex-1 flex-col gap-6 overflow-hidden">
      ...
      <div className="flex justify-end">
        <Button asChild variant="outline">
          <Link to={APP_ROUTES.faqs.path}>{t('common.actions.back', { ns: 'translation' })} {entityPlural}</Link>
        </Button>
      </div>
    </section>
  )
